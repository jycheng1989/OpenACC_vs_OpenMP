...
!OpenACC with reduction and atomic
module particle
   implicit none
...
Program test
   ...
   do i = 1, testLoopCount
      ...
      grid2 = 0.0
      grid3 = 0.0
      !$acc update device(grid1, grid2, grid3)
      startTime = MPI_WTIME()
      call testKernel()
      endTime = MPI_WTIME()
      !$acc update host(grid1, grid2, grid3)
   ...
   !endif
   write(*,*)'OpenACC: total time is', totalTime
...
subroutine testKernel()
   ...
   !$acc reduction(+: outOfBoundaryOfParticle1, outOfBoundaryOfParticle2, outOfBoundaryOfParticle3) &
   !$acc copy(outOfBoundaryOfParticle1, outOfBoundaryOfParticle2, outOfBoundaryOfParticle3), copyin(dGrid1, dGrid2, dGrid3) 
...
subroutine initializeParticle()
   ...
   !$acc enter data create(particle1, particle2, particle3)
   call random_number(particle1)
   call random_number(particle2)
   call random_number(particle3)
   !$acc update device(particle1, particle2, particle3)
...
subroutine initializeGrid()
   ...
   !$acc enter data create(grid1, grid2, grid3)
   grid1 = 0.0
   grid2 = 0.0
   grid3 = 0.0
   !$acc update device(grid1, grid2, grid3)
...
subroutine finalizeParticle()
   use particle
   implicit none
   !$acc exit data delete(particle1, particle2, particle3)
...
subroutine finalizeGrid
   use grid
   implicit none
   !$acc exit data delete(grid1, grid2, grid3)
